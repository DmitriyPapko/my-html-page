...
/* ==== Menu & toggles ==== */
const menu=document.getElementById('menu'); const btnStart=document.getElementById('btnStart'); const btnRestart=document.getElementById('btnRestart'); const btnMute=document.getElementById('btnMute'); const btnPause=document.getElementById('btnPause'); const btnFog=document.getElementById('btnFog');
let paused=true; let muted=false;
btnStart.onclick=()=>{ paused=false; menu.style.display='none'; };
btnRestart.onclick=()=>{ resetGame(); paused=false; menu.style.display='none'; };
btnMute.onclick=()=>{ muted=!muted; btnMute.textContent=muted?'Звук: выкл':'Звук: вкл'; };
btnPause.onclick=()=>{ paused=!paused; menu.style.display=paused?'flex':'none'; };
let fogEnabled=true; btnFog.onclick=()=>{ fogEnabled=!fogEnabled; btnFog.textContent=fogEnabled?'No Fog':'Fog'; };
...
function genBlockers(){
  blockers.length=0;
  for(let i=0;i<4;i++){ const x= 2000+i*2500 + rand(-400,400); for(let y=600;y<world.height-600;y+=180){ blockers.push({x:x+rand(-60,60),y:y,r:38}); } }
  for(let i=0;i<70;i++){ blockers.push({x:rand(600,world.width-600), y:rand(600,world.height-600), r: clamp(rand(24,60),24,60)}); }
}
function drawTerrain(){ ctx.save(); ctx.scale(world.zoom,world.zoom);
  const step=96; const startX=Math.floor(world.camX/step)*step, startY=Math.floor(world.camY/step)*step; const endX=world.camX+cvs.width/world.zoom, endY=world.camY+cvs.height/world.zoom;
  for(let y=startY;y<=endY;y+=step){ for(let x=startX;x<=endX;x+=step){ const h=((Math.sin(x*0.0006)+Math.cos(y*0.0007))*0.5+0.5); let r=18,g=58,b=20; if(h>0.9){ r=80;g=82;b=90; } ctx.fillStyle=`rgb(${r},${g},${b})`; ctx.fillRect(x-world.camX,y-world.camY,step,step); } }
  // реки/лес
  for(const b of blockers){ const s=worldToScreen(b.x,b.y); ctx.fillStyle='rgba(30,80,120,.9)'; ctx.beginPath(); ctx.arc(s.x,s.y,b.r*world.zoom,0,6.283); ctx.fill(); ctx.strokeStyle='rgba(0,0,0,.35)'; ctx.stroke(); }
  ctx.restore(); }
function isBlocked(wx,wy){ for(const b of blockers){ if(dist2(wx,wy,b.x,b.y)<= (b.r+10)*(b.r+10)) return true; } return false; }
...
function beep(freq=440,dur=0.08,type='triangle',gain=0.06){ if(muted||!AC) return; const o=AC.createOscillator(), g=AC.createGain(); o.type=type; o.frequency.value=freq; o.connect(g); g.connect(AC.destination); g.gain.value=gain; o.start(); g.gain.exponentialRampToValueAtTime(0.0001, AC.currentTime+dur); o.stop(AC.currentTime+dur); }
...
class Structure extends Entity{ constructor(x,y,owner,kind='hq',ghost=false){ super(x,y,owner); this.kind=kind; this.isGhost=ghost; this.radius=36; this.maxHp=ghost?200:520; this.hp=ghost?40:520; this.queue=[]; this.qTime=0; this.vision=520; this.regen=0.3; }
  update(dt){ if(this.isGhost){ this.hp=Math.min(this.maxHp,this.hp+15*dt); if(this.hp>=this.maxHp) this.isGhost=false; return; }
...
function tryCast(slot){ const h=players[0].hero; if(!h||h.dead) return;
  if(slot===1 && h.cd1<=0){ if(h.heroClass==='paladin'){ if(h.mp<40) return; h.mp-=40; const R=180; for(const u of players[0].units){ if(u.dead) continue; if(Math.hypot(u.x-h.x,u.y-h.y)<=R){ u.hp=Math.min(u.maxHp,u.hp+90); } } beep(660,0.08,'triangle',0.05); } 
...
function updateBuildingPanel(){ const selected=players[0].structures.filter(s=>s.selected && !s.isGhost && s.owner===0); if(selected.length!==1){ buildingPanel.style.display='none'; buildingPanel.innerHTML=''; return; } const b=selected[0]; buildingPanel.style.display='flex'; buildingPanel.innerHTML='';
...
function renderWorkerBuildPanel(){ const selected=players[0].units.filter(u=>u.selected && u.type==='worker'); if(selected.length===0){ workerBuild.style.display='none'; workerBuild.innerHTML=''; return; } workerBuild.style.display='flex'; workerBuild.innerHTML='';
...
function aiThink(dt,id){
  const P=players[id]; if(!P.ai) return; if(!P.aiPlan) aiInitPlan(P);
  ...
  if(wcount<6 && HQ){ if(P.res.rice>=50 && P.res.water>=12 && P.units.filter(u=>!u.dead&&!u.isHero).length<POP_CAP){ HQ.queue.push('worker'); } }
  // training
  const barr=P.structures.find(s=>s.kind==='barracks' && !s.isGhost), rng=P.structures.find(s=>s.kind==='range' && !s.isGhost), mb=P.structures.find(s=>s.kind==='mBarracks' && !s.isGhost);
  ...
  // army rally / attack (приоритет бить героя игрока)
...
function loop(t){ const dt=Math.min(0.033,(t-last)/1000); last=t; simTime+=dt;
