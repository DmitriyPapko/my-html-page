import { Entity, Unit, Structure, getById, enemiesFor, allUnits, allStructures, nearestNode, lootFromTier, genId } from "./entities.js";
...
let simTime = 0;
      ...
      class ResourceNode { constructor(x, y, type = 'rice') { this.id = genId(); this.x = x; this.y = y; this.type = type; this.radius = 44; } draw() { if (!isExplored(this.x, this.y)) return; const s = worldToScreen(this.x, this.y); const r = this.radius * world.zoom; if (this.type === 'rice') { ctx.fillStyle = '#2a4e2a'; ctx.beginPath(); ctx.arc(s.x, s.y, r, 0, 6.283); ctx.fill(); } else { ctx.fillStyle = '#1a4e6e'; ctx.beginPath(); ctx.arc(s.x, s.y, r, 0, 6.283); ctx.fill(); } } }
